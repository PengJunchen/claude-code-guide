...
- Maximum pool size too low ‚Ü?connection starvation
- Connection acquisition timeout too low ‚Ü?false failures  
- Idle timeout misconfigured ‚Ü?excessive connection churn
- Connection lifetime exceeding database timeout ‚Ü?stale connections
- Pool size not accounting for concurrent workers ‚Ü?resource contention
```
